...
const TOOLS = [
  ...
  },
  {
    ...
    description: "แสดงรหัสนักเรียนที่ไม่ผ่านเกณฑ์ PBL (มีด้านใดได้ 1) พร้อมด้านที่อ่อน โดยไม่คืนชื่อนักเรียนหรือข้อความหมายเหตุ — สำหรับติดตาม/แจ้งเตือน/ช่วยเหลือ",
    inputSchema: {
      type: "object",
      properties: {
        term: { type: "string", description: "รหัสภาคเรียน" },
  ...
  },
  {
    ...
    description: "พัฒนาการสมรรถนะ PBL ตามรหัสนักเรียน: คะแนน 5 ด้านในแต่ละโปรเจกต์/หน่วยของภาคเรียน โดยไม่คืนชื่อนักเรียน",
    inputSchema: {
      type: "object",
      properties: {
        term: { type: "string", description: "รหัสภาคเรียน" },
...
const CODEX_READ_ONLY_TOOL_NAMES = new Set([
  "atlas_list_terms",
  "atlas_classroom_kpi",
  ...
  "atlas_plc_sessions",
  "atlas_action_items",
  "atlas_plc_effectiveness",
  "atlas_plc_coverage_gap",
  "atlas_plc_timeline",
  "atlas_cross_plc_opportunities",
  "atlas_plc_recommendations",
  "atlas_pbl_summary",
  "atlas_pbl_class_profile",
  "atlas_pbl_failing",
  "atlas_pbl_student",
...
const CODEX_READ_ONLY_TOOLS = TOOLS.filter((tool) =>
  CODEX_READ_ONLY_TOOL_NAMES.has(tool.name)
);
  ...
  try {
    switch (name) {
      ...
      case "atlas_pbl_failing": {
        ...
        const assess = await fetchAllRows((from, to) => admin.from("pbl_assessments")
          .select("student_id, project_id, com_score, think_score, problem_score, life_score, tech_score")
      ...
      case "atlas_pbl_student": {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ term: args.term, student_id: args.student_id, projects: projectsOut }, null, 2) }] };
      }
      ...
      case "atlas_pbl_unit_crosscheck": {
        ...
        const assess = await fetchAllRows((from, to) => admin.from("pbl_assessments")
          .select("student_id, project_id, com_score, think_score, problem_score, life_score, tech_score, overall_result")
          .in("project_id", ids)
          .order("id")
          .range(from, to), "pbl_assessments");
        ...
        (assess || []).forEach((a: any) => {
          ...
          if (!pblByKey[key]) pblByKey[key] = { student_id: a.student_id, grade_level: p.grade_level, classroom: p.classroom, pcts: [], worst: 2 };
        ...
        Object.values(pblByKey).forEach((s: any) => {
          ...
          classesMap[cls].students.push({ student_id: s.student_id, pbl_pct, pbl_result, unit_pct, gap, pattern, flags });
        });
...
Deno.serve(async (req: Request) => {
  ...
  const expectedKey = Deno.env.get("WORANAT_CODEX_MCP_KEY");
  ...
  try {
    switch (method) {
      ...
      case "ping":
        ...
        break;
      case "tools/list":
        result = { tools: CODEX_READ_ONLY_TOOLS };
        break;
      case "tools/call":
        if (
          !params?.name ||
          !CODEX_READ_ONLY_TOOL_NAMES.has(params.name)
        ) {
          return new Response(
            JSON.stringify({
              jsonrpc: "2.0",
              ...
              error: {
                code: -32601,
                message: "Tool is not available in read-only Codex MCP",
              },
            }),
            {
              status: 200,
