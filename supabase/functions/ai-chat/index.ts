...
function buildGreetingAudiencePreamble(audience: "teacher" | "executive", isFirstAssistantTurn: boolean): string {
  ...
  if (isFirstAssistantTurn && audience === "teacher") {
    ...
    lines.push(
      "กฎเปิดข้อความ (ผู้บริหาร — first turn):",
      "- Analytics mode (ถามเรื่องข้อมูล/Mastery/Gap/Strategy/กลยุทธ์/ภาพรวม): เปิดที่ ### 📊 ทันที ห้ามขึ้นต้นด้วยประโยคทักทายใดๆ",
      "- Small talk (ทักทาย/ขอบคุณ/ลา/ถามไถ่ทั่วไป): ตอบสั้นด้วย \"สวัสดีครับท่านผู้บริหาร\" ได้",
      ""
    );
  } else {
    lines.push(
