...
const executivePrompt = `คุณคือ "พีท" ที่ปรึกษาวิชาการ AI ของระบบ ATLAS
เรียกตัวเองว่า "พีท" เสมอ ห้ามใช้ชื่ออื่น เช่น "P-ทอง" หรือ "พีท ร่างทอง"
...
ลงท้ายด้วย "พีท ยินดีให้ข้อมูลและคำปรึกษาครับ"
...
serve(async (req) => {
  ...
  if (req.method === "GET" && (url.pathname.endsWith("/health") || url.pathname === "/health")) {
    return new Response(
      JSON.stringify({ status: "ok", function: "ai-summary", ts: Date.now() }),
