-- ============================================================
-- ปัญหา: RLS policy teacher_own_rows = (teacher_id = auth.uid())
--        แถวที่ teacher_id เป็น NULL จึงไม่มีครูคนใดเห็นในแอปเลย
--        สาเหตุ: สาขา PBL ใน v_research_candidates_n8n เขียน NULL::text AS teacher_id
--        (ตาราง pbl_projects ไม่มีคอลัมน์ teacher_id) → หัวข้อ 7 รายการของครู 6 ท่านมองไม่เห็น
-- แก้ 3 ส่วน: (1) typo ชื่อครู  (2) view resolve teacher_id  (3) backfill ย้อนหลัง
-- ============================================================

-- ---------- ส่วนที่ 1: แก้ typo ชื่อครู "ริทราย" -> "รินทราย" (ตก น) ----------
-- ยืนยันว่าเป็นคนเดียวกัน: สอน ป.2 นามสกุลสาลีจันทร์ ตรงกับ profiles และ teaching_logs (164 บันทึก)
UPDATE pbl_projects
   SET teacher_name = 'ครูรินทราย สาลีจันทร์'
 WHERE teacher_name = 'ครูริทราย สาลีจันทร์';

UPDATE classroom_research_suggestions
   SET teacher_name = 'ครูรินทราย สาลีจันทร์'
 WHERE teacher_name = 'ครูริทราย สาลีจันทร์';

-- ---------- ส่วนที่ 2: view — สาขา PBL resolve teacher_id จาก profiles ----------
...
UNION ALL
 SELECT
    ...
    'PBL'::text AS subject,
    ps.teacher_name,
    -- ⭐ แก้ 12 ส.ค. 2569: เดิมเป็น NULL::text ทำให้ครูมองไม่เห็นหัวข้อในแอป (ติด RLS)
    -- pbl_projects ไม่มีคอลัมน์ teacher_id จึง resolve จาก profiles ด้วยชื่อที่ normalize แล้ว
    -- (ตัดคำนำหน้า ครู/นาง/นางสาว/น.ส./นาย และช่องว่างทั้งหมด) ใช้ scalar subquery กันแถวบานปลาย
    ( SELECT (p.user_id)::text
        FROM profiles p
       WHERE (p.full_name <> ''::text)
         AND regexp_replace(regexp_replace(p.full_name, '^(ครู|นางสาว|นาง|น\.ส\.|นาย)\s*'::text, ''::text), '\s+'::text, ''::text, 'g'::text)
           = regexp_replace(regexp_replace(ps.teacher_name, '^(ครู|นางสาว|นาง|น\.ส\.|นาย)\s*'::text, ''::text), '\s+'::text, ''::text, 'g'::text)
...
UNION ALL
( SELECT DISTINCT ON (ap.teacher_name, ap.classroom, ap.subject, ap.issue_type) ap.issue_type,
    ap.severity,
    ap.grade_level,
    ap.classroom,
    ap.subject,
    COALESCE(ap.teacher_name, '-'::text) AS teacher_name,
    ...
    ((((((('action_plan_items #'::text || ap.id) || ' ('::text) || ap.issue_type) || ') เปิดค้าง '::text) || (CURRENT_DATE - ap.run_date)) || ' วัน'::text) || COALESCE((' | '::text || ap.detail), ''::text)) AS evidence_summary
   FROM action_plan_items ap
  WHERE ((ap.issue_type = ANY (ARRAY['FlatScore'::text, 'MasteryDrop'::text])) AND (ap.status = ANY (ARRAY['open'::text, 'watching'::text])) AND ((CURRENT_DATE - ap.run_date) >= 7) AND (NOT (COALESCE(ap.teacher_name, '-'::text) IN ( SELECT selected_teachers.teacher_name
           FROM selected_teachers))))
  ORDER BY ap.teacher_name, ap.classroom, ap.subject, ap.issue_type, ap.run_date DESC)
  ORDER BY 1, 3, 4;

-- ---------- ส่วนที่ 3: backfill teacher_id ย้อนหลังให้แถวที่ค้างอยู่ ----------
UPDATE classroom_research_suggestions c
   SET teacher_id = p.user_id
  FROM profiles p
 WHERE c.teacher_id IS NULL
   AND p.full_name <> ''
   AND regexp_replace(regexp_replace(p.full_name, '^(ครู|นางสาว|นาง|น\.ส\.|นาย)\s*', ''), '\s+', '', 'g')
     = regexp_replace(regexp_replace(c.teacher_name, '^(ครู|นางสาว|นาง|น\.ส\.|นาย)\s*', ''), '\s+', '', 'g');
